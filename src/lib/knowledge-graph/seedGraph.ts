...
export const seedKnowledgeGraph = async (): Promise<void> => {
  try {
    ...
    await runQuery(`
      ...
      CREATE (:Supplier {name: 'USA', region: 'Americas', daily_export_mbpd: 0.8, reliability_score: 92})
    ...
    await runQuery(`
      ...
      CREATE (:Port {name: 'Visakhapatnam Port', state: 'Andhra Pradesh', capacity_mbpd: 0.5, lat: 17.6868, lng: 83.2185})
    `)
    ...
    await runQuery(`
      ...
      CREATE (:Refinery {name: 'IOC Panipat', operator: 'IOC', capacity_mbpd: 0.30, lat: 29.3909, lng: 76.9635})
      CREATE (:Refinery {name: 'HPCL Vizag', operator: 'HPCL', capacity_mbpd: 0.17, lat: 17.7231, lng: 83.3012})
    `)
    ...
    `)
    await runQuery(`
      MATCH (s:Supplier {name: 'USA'}), (c:Chokepoint {name: 'Cape of Good Hope'})
      CREATE (s)-[:SUPPLIES_THROUGH {volume_pct: 100, transit_days: 32}]->(c)
    `)
    ...
    `)
    await runQuery(`
      MATCH (p:Port {name: 'Visakhapatnam Port'}), (r:Refinery {name: 'HPCL Vizag'})
      CREATE (p)-[:FEEDS {pipeline_km: 9, mode: 'pipeline'}]->(r)
    `)
